...
Anna's Archive's DDoS protection sometimes sends a JavaScript challenge instead of search results. The package detects the challenge before parsing, waits briefly, and retries with the same HTTP cookie session. Preserving the paired DDoS-Guard cookies lets a later request use any admission state that Anna's Archive granted during an earlier attempt. The package reports each retry in the echo area. If all attempts fail, it signals a search error.
...
The package reports "No results found" only when Anna's Archive returns its explicit "No files found" marker. DDoS-Guard pages, server errors, and unparseable pages are reported separately.
