#+title: annas-archive: Search and download from Anna's Archive
#+author: Pablo Stafforini
#+email: pablo@stafforini.com
#+language: en
#+options: ':t toc:t author:t email:t num:t
#+startup: content
#+export_file_name: annas-archive.info
#+texinfo_filename: annas-archive.info
#+texinfo_dir_category: Emacs misc features
#+texinfo_dir_title: Anna's Archive: (annas-archive)
#+texinfo_dir_desc: Search and download from Anna's Archive
#+texinfo_header: @c AUTO-GENERATED from README.org via ox-texinfo — do not edit manually.

This manual describes the features and customization options for the Emacs Lisp file =annas-archive.el=.

* Overview
:PROPERTIES:
:CUSTOM_ID: h:overview
:END:

=annas-archive.el= provides Emacs integration for [[https://en.wikipedia.org/wiki/Anna%27s_Archive][Anna's Archive]], the largest search engine for shadow libraries. It lets you search for books and papers by title, ISBN, or DOI, browse results in a formatted completion interface, and download files---all without leaving Emacs.

The development repository is on [[https://github.com/benthamite/annas-archive][GitHub]].

Anna's Archive does not provide a search-results API. When the [[https://github.com/benthamite/chrome-to-eww][Chrome to Emacs bridge]] is available, the default search backend loads the HTML search route in an inactive tab of an existing Google Chrome window. The extension runs JavaScript challenges in the admitted browser session, returns the final DOM, and closes only the tab it created. It never starts Chrome, creates a Chrome window, selects a tab, or focuses a window. The package validates the HTML, renders it with =shr=, extracts bibliographic metadata (title, file type, size, language, and year), and presents the results via ~completing-read~. It does not use =eww= for searches.

Two download mechanisms are available:

- *Programmatic download via the fast download API.* When ~annas-archive-secret-key~ is set, the package calls the Anna's Archive JSON API to obtain a direct download URL, then retrieves the file asynchronously within Emacs and saves it to ~annas-archive-downloads-dir~ ([[#h:download-options][Download options]]).

- *External browser fallback.* When the API key is not set, the package opens the selected item page in the system's default browser. When a programmatic download fails, ~annas-archive-when-download-fails~ controls the fallback ([[#h:download-failure-handling][Download failure handling]]).

The Elisp code depends only on libraries bundled with Emacs. The optional Chrome backend requires an existing Google Chrome window and the Chrome to Emacs extension and native host. Without the bridge, the default ~auto~ backend uses direct HTTP, which cannot execute JavaScript challenges.

** DOI support
:PROPERTIES:
:CUSTOM_ID: h:doi-support
:END:

When the search string is a DOI (e.g. =10.1145/1458082.1458150=), the package searches Anna's Archive's journals index for the exact DOI. It presents every matching file through the same completion interface used for other searches.

** Breaking change (February 2026)
:PROPERTIES:
:CUSTOM_ID: h:breaking-change
:END:

As of February 2026, Anna's Archive changed its download flow so that the "Download" links on item pages are now JavaScript-driven. Programmatic downloads now use the [[https://annas-archive.gl/dyn/api/fast_download.json][fast download JSON API]], which requires a *secret key*.

If you were using ~annas-archive-use-fast-download-links~ and/or ~annas-archive-use-eww~, replace them with ~annas-archive-secret-key~:

#+begin_src emacs-lisp
;; Before:
(setq annas-archive-use-fast-download-links t)
(setq annas-archive-use-eww t)

;; After:
(setopt annas-archive-secret-key "YOUR_SECRET_KEY")
#+end_src

To find your key, log into Anna's Archive with your paid membership and visit your [[https://annas-archive.gl/account/][account page]]. Both ~annas-archive-use-fast-download-links~ and ~annas-archive-use-eww~ are now obsolete.

* Installation
:PROPERTIES:
:CUSTOM_ID: h:installation
:END:

** Manual installation
:PROPERTIES:
:CUSTOM_ID: h:manual-installation
:END:

Clone the repository and add it to your load path:

#+begin_src emacs-lisp
(add-to-list 'load-path "/path/to/annas-archive/")
(require 'annas-archive)
#+end_src

** Installation with use-package
:PROPERTIES:
:CUSTOM_ID: h:use-package-installation
:END:

If you use ~use-package~, add one of the following snippets to your init file:

#+begin_src emacs-lisp
;; with vc (Emacs 30+)
(use-package annas-archive
  :vc (:url "https://github.com/benthamite/annas-archive"))

;; with elpaca
(use-package annas-archive
  :ensure (:host github :repo "benthamite/annas-archive"))

;; with straight
(use-package annas-archive
  :straight (:host github :repo "benthamite/annas-archive"))

;; with quelpa
(use-package annas-archive
  :quelpa (annas-archive :fetcher github :repo "benthamite/annas-archive"))
#+end_src

** Chrome bridge
:PROPERTIES:
:CUSTOM_ID: h:chrome-bridge-installation
:END:

For reliable searches when Anna's Archive enables its JavaScript challenge, install the [[https://github.com/benthamite/chrome-to-eww][Chrome to Emacs bridge]] in the Chrome profile that should perform searches. Follow that project's installation instructions to load the extension and register its native host, then reload Chrome once. The extension creates search tabs with ~active: false~ and removes them after it returns the DOM.

The default ~auto~ backend detects the bridge's private Unix-domain socket. Set ~annas-archive-search-backend~ to ~chrome~ to require the bridge or to ~direct~ to bypass it.

* User options
:PROPERTIES:
:CUSTOM_ID: h:user-options
:END:

** Domain resolution
:PROPERTIES:
:CUSTOM_ID: h:domain-resolution
:END:

#+vindex: annas-archive-home-url-override

The package resolves the current Anna's Archive URL from the [[https://en.wikipedia.org/wiki/Anna%27s_Archive][Wikipedia article]] on first use and caches it for the rest of the Emacs session. Normally you do not need to configure the home URL.

The user option ~annas-archive-home-url-override~ can be set to a URL string when you need to bypass Wikipedia temporarily, for example during a Wikipedia outage or while testing a specific mirror. The default value is ~nil~.

** Download options
:PROPERTIES:
:CUSTOM_ID: h:download-options
:END:

#+vindex: annas-archive-secret-key
#+vindex: annas-archive-downloads-dir
#+vindex: annas-archive-post-download-hook

The user option ~annas-archive-secret-key~ specifies the secret key for the Anna's Archive fast download API. When set to a non-empty string, it enables programmatic downloads directly within Emacs: the package calls the API to get a direct download URL, retrieves the file asynchronously, and saves it to disk. The default value is ~nil~, which means programmatic downloads are disabled and the package will fall back to opening the download URL in the system browser.

To obtain a secret key, you need a paid [[https://annas-archive.gl/donate][membership]] on Anna's Archive. Once logged in, visit the [[https://annas-archive.gl/account/][account page]] to find your key.

The user option ~annas-archive-downloads-dir~ specifies the directory where programmatically downloaded files are saved. The default value is =~/Downloads/=. This option is only relevant when ~annas-archive-secret-key~ is set; external browser downloads go to whatever location the browser is configured to use.

The user option ~annas-archive-post-download-hook~ is a hook run after downloading a file. Each function on the hook is called with the download URL as its first argument. When the file was downloaded programmatically (i.e. via the fast download API), the destination file path is passed as the second argument. This is useful for post-processing, such as automatically renaming or moving the downloaded file. The default value is ~nil~.

** Download failure handling
:PROPERTIES:
:CUSTOM_ID: h:download-failure-handling
:END:

#+vindex: annas-archive-when-download-fails

The user option ~annas-archive-when-download-fails~ controls what happens when a programmatic download fails. It accepts one of three values:

- ~external~ (the default): open the download URL in the system's default browser so the user can complete the download manually.
- ~error~: signal an error, which is useful in non-interactive scripts where silent failure is undesirable.
- ~nil~: fail silently with only a message in the echo area.

A programmatic download can fail for several reasons: the fast download API may return an error (e.g. daily quota exhausted, invalid key), the remote server may return an HTML challenge page instead of the file, or the connection may time out.

** Search filtering
:PROPERTIES:
:CUSTOM_ID: h:search-filtering
:END:

#+vindex: annas-archive-included-file-types
#+vindex: annas-archive-retry-with-all-file-types
#+vindex: annas-archive-search-retries
#+vindex: annas-archive-search-retry-delay
#+vindex: annas-archive-search-backend
#+vindex: annas-archive-search-mirrors
#+vindex: annas-archive-chrome-bridge-socket

The user option ~annas-archive-included-file-types~ specifies which file extensions to include when displaying search results. The value is a list of lowercase extension strings. The default includes all types supported by the package: =pdf=, =epub=, =fb2=, =mobi=, =cbr=, =djvu=, =cbz=, =txt=, and =azw3=.

If you only want PDF and EPUB results, for example:

#+begin_src emacs-lisp
(setopt annas-archive-included-file-types '("pdf" "epub"))
#+end_src

The user option ~annas-archive-retry-with-all-file-types~ controls whether the package automatically retries a search with all supported file types when the filtered search returns no results. When set to ~t~ (the default), the package prompts with a =y-or-n= question before retrying. Set it to ~nil~ to disable this behavior.

The user option ~annas-archive-search-backend~ controls how the package obtains search HTML. Its default value, ~auto~, uses the Chrome bridge when its socket exists and direct HTTP otherwise. Set it to ~chrome~ to require the bridge or to ~direct~ to require Emacs's URL library. The Chrome backend can execute Anna's Archive's browser challenge. The direct backend cannot.

The user option ~annas-archive-search-mirrors~ sets the ordered hostnames tried after transient failures. The package rotates mirrors instead of repeatedly requesting one blocked host. ~annas-archive-chrome-bridge-socket~ overrides the bridge's standard private socket path when a custom installation uses another location.

Anna's Archive sometimes returns a DDoS-Guard challenge or a temporary server-error page in place of the search results. The package detects these responses and retries instead of reporting that the search has no results. ~annas-archive-search-retries~ sets the maximum number of retries (10 by default), while ~annas-archive-search-retry-delay~ sets the delay between attempts in seconds (1 by default). Each retry is reported in the echo area.

The package reports "No results found" only when the page contains Anna's Archive's explicit empty-search marker. If the page has neither that marker nor parseable results, it reports that the response could not be parsed. A file-type filter that excludes every returned item also has its own message.

** Column widths
:PROPERTIES:
:CUSTOM_ID: h:column-widths
:END:

#+vindex: annas-archive-title-column-width
#+vindex: annas-archive-type-column-width
#+vindex: annas-archive-size-column-width
#+vindex: annas-archive-year-column-width
#+vindex: annas-archive-language-column-width

The following user options control the column widths in the formatted completion candidates displayed by ~annas-archive-download~ ([[#h:searching-and-downloading][Searching and downloading]]). Each value is an integer specifying the number of characters.

| User option                              | Default | Description     |
|------------------------------------------+---------+-----------------|
| ~annas-archive-title-column-width~       |     100 | Title column    |
| ~annas-archive-type-column-width~        |       5 | File type       |
| ~annas-archive-size-column-width~        |       8 | File size       |
| ~annas-archive-year-column-width~        |       4 | Publication year |
| ~annas-archive-language-column-width~    |      20 | Language        |

You may want to reduce ~annas-archive-title-column-width~ if you work with a narrow frame, or increase ~annas-archive-language-column-width~ if you frequently encounter entries with multiple languages.

* Commands
:PROPERTIES:
:CUSTOM_ID: h:commands
:END:

** Searching and downloading
:PROPERTIES:
:CUSTOM_ID: h:searching-and-downloading
:END:

#+findex: annas-archive-download

The command ~annas-archive-download~ is the main entry point for the package. When called interactively via =M-x annas-archive-download=, it prompts for a search string. The search string can be:

- A book title, author name, or any descriptive text.
- An ISBN.
- A DOI (e.g. =10.1145/1458082.1458150=) for academic papers.

The command obtains Anna's Archive's HTML search results through ~annas-archive-search-backend~, parses titles, file types, sizes, languages, and publication years, and presents them in a formatted ~completing-read~ interface. The Chrome backend uses an inactive tab in an existing window and does not select it or change window focus. A DDoS-Guard challenge or temporary server error causes mirror rotation and a bounded automatic retry rather than a false "No results found" message. A malformed page produces an error. After you select a result, the package passes its MD5 to the fast download API or opens the item page in the default browser.

When the search string is a DOI, the command makes an exact DOI search in the journals index ([[#h:doi-support][DOI support]]).

When called non-interactively, the command expects a non-empty string argument and does not prompt. It signals an error if the argument is ~nil~ or empty. This makes it suitable for use in Lisp code:

#+begin_src emacs-lisp
(annas-archive-download "10.1145/1458082.1458150")
#+end_src

** Selecting from search results
:PROPERTIES:
:CUSTOM_ID: h:selecting-from-results
:END:

#+findex: annas-archive-collect-results

The command ~annas-archive-collect-results~ parses the current SHR-rendered buffer as an Anna's Archive search results page, filters the results by file type, and presents them for selection via ~completing-read~. It accepts an optional TYPES argument (a list of lowercase extension strings) to override the default filter from ~annas-archive-included-file-types~ ([[#h:search-filtering][Search filtering]]).

This command is used internally by the ~annas-archive-download~ workflow. Most users should call ~annas-archive-download~ instead.

* Functions
:PROPERTIES:
:CUSTOM_ID: h:functions
:END:

** Parsing search results
:PROPERTIES:
:CUSTOM_ID: h:parsing-functions
:END:

#+findex: annas-archive-parse-results
#+findex: annas-archive-get-links

The function ~annas-archive-parse-results~ parses the current SHR-rendered buffer, which should contain Anna's Archive search results, and returns a list of plists. Each plist has the keys ~:title~, ~:url~, ~:type~, ~:size~, ~:language~, and ~:year~. This function is the backbone of the result-selection interface and can be used in custom Lisp code to extract structured data from a rendered search results page.

The function ~annas-archive-get-links~ extracts all hyperlinks from the current SHR-rendered buffer as an alist of =(TITLE . URL)= pairs. It walks the buffer's text properties to find regions with the ~shr-url~ property. It is primarily used by ~annas-archive-parse-results~.

** Download helpers
:PROPERTIES:
:CUSTOM_ID: h:download-helpers
:END:

#+findex: annas-archive-download-file-internally
#+findex: annas-archive-download-file-externally
#+findex: annas-archive-download-file-callback
#+findex: annas-archive-save-file
#+findex: annas-archive-handle-download-failure
The function ~annas-archive-download-file-internally~ initiates an asynchronous download of the file at a given URL using ~url-retrieve~. The actual file saving is handled by the callback returned by ~annas-archive-download-file-callback~ ([[#h:download-options][Download options]]).

The function ~annas-archive-download-file-externally~ opens the given URL in the system's default browser using ~browse-url-default-browser~ and runs ~annas-archive-post-download-hook~ with the URL as argument.

The function ~annas-archive-download-file-callback~ returns a closure suitable for use as a ~url-retrieve~ callback. The closure handles HTTP errors, determines the file extension from the response (trying the redirect URL, Content-Type header, and original URL in turn), strips HTTP headers from the response buffer, detects HTML challenge pages, and saves the file via ~annas-archive-save-file~.

The function ~annas-archive-save-file~ writes the current buffer contents to the given path using binary (no-conversion) coding, creating the parent directory if needed. It then runs ~annas-archive-post-download-hook~ with the URL and file path.

The function ~annas-archive-handle-download-failure~ dispatches on the value of ~annas-archive-when-download-fails~ to either open the URL externally, signal an error, or fail silently ([[#h:download-failure-handling][Download failure handling]]).

* Troubleshooting
:PROPERTIES:
:CUSTOM_ID: h:troubleshooting
:END:

** Anna's Archive domain changes
:PROPERTIES:
:CUSTOM_ID: h:domain-changes
:END:

Anna's Archive frequently changes its domain name. The package resolves the current URL from Wikipedia automatically and caches it for the current Emacs session. If URL resolution fails, check whether Wikipedia is reachable and whether the article still lists the current URL in the infobox.

If you need a temporary manual URL, set ~annas-archive-home-url-override~. The old ~annas-archive-home-url~ option is obsolete and no longer controls URL resolution.

** DDoS-Guard challenges
:PROPERTIES:
:CUSTOM_ID: h:ddos-guard-challenges
:END:

Anna's Archive's DDoS protection sometimes sends a JavaScript challenge instead of search results. The Chrome backend uses the existing admitted browser session, creates an inactive tab, waits for a validated result or explicit empty page, and closes the tab. It never selects the tab or changes window focus. The package detects challenge and server-error pages, rotates to another configured mirror, and retries. If all attempts fail, it signals a search error.

The ~direct~ backend detects the same failures, but Emacs's URL library cannot execute the JavaScript challenge. Use that backend only when direct requests work in your environment.

You can change the retry limit and delay with ~annas-archive-search-retries~ and ~annas-archive-search-retry-delay~ ([[#h:search-filtering][Search filtering]]).

** No results found
:PROPERTIES:
:CUSTOM_ID: h:no-results
:END:

The package reports "No results found" only when a completed Anna's Archive search page returns its explicit "No files found" marker without any challenge or server-error marker. Parseable results take precedence over stale challenge titles. DDoS-Guard pages, server errors, and unparseable pages are reported separately.

If Anna's Archive returned results but none match ~annas-archive-included-file-types~, the package says that no results match the configured file types. It will prompt you to retry with all file types if ~annas-archive-retry-with-all-file-types~ is ~t~ ([[#h:search-filtering][Search filtering]]).

** API errors
:PROPERTIES:
:CUSTOM_ID: h:api-errors
:END:

The fast download API can return several errors. The package translates them into user-friendly messages:

| API error                          | Meaning                                       |
|------------------------------------+-----------------------------------------------|
| Invalid secret key                 | The value of ~annas-archive-secret-key~ is wrong |
| Not a member                       | Your account lacks a paid membership           |
| No downloads left                  | Daily download quota exhausted                 |
| Record not found                   | The file does not exist in Anna's Archive      |
| Invalid domain_index or path_index | File unavailable for fast download             |
| Error during fetching              | Server-side error                              |

** Reporting issues
:PROPERTIES:
:CUSTOM_ID: h:reporting-issues
:END:

If you encounter problems not covered here, please open an issue on the [[https://github.com/benthamite/annas-archive][GitHub repository]].

* Indices
:PROPERTIES:
:CUSTOM_ID: h:indices
:END:

** Function index
:PROPERTIES:
:INDEX: fn
:CUSTOM_ID: h:function-index
:END:

** Variable index
:PROPERTIES:
:INDEX: vr
:CUSTOM_ID: h:variable-index
:END:
