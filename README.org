...
Anna's Archive redirects the SciDB URL of a DOI that it holds no record of to a journals search page. When that happens, the package reports that there is no SciDB record for the DOI and falls back to the normal search selection flow, so you can still pick a matching item if one exists.
...
When the search string is a DOI, the command skips the search step and navigates directly to the SciDB page for that DOI, proceeding straight to download. If Anna's Archive has no SciDB record for the DOI and redirects to a journals search page, the command returns to the search selection flow ([[#h:doi-support][DOI support]]).
...
When called interactively, it displays a message indicating the download directory. When called non-interactively (e.g. from a hook), it waits through intermediate =eww= renders until the buffer reaches a valid MD5 or SciDB download page, then silently kills the =eww= buffer after initiating the download.
