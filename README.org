...
Anna's Archive frequently changes its domain name. To update the package default for ~annas-archive-home-url~ from the current URL listed in the [[https://en.wikipedia.org/wiki/Anna%27s_Archive][Wikipedia article]], run this from the package checkout:

#+begin_src shell
emacs -Q --batch -L . -l annas-archive.el \
  --eval '(annas-archive-update-home-url-default)'
#+end_src

The command rewrites the default string in ~annas-archive.el~. It does not save anything to your personal Custom file.
